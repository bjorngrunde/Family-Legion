class Admin::RolesController < AdminController
  ...
  def add_role
    ...
    return redirect_back(fallback_location: dashboard_path, flash: flashy(:positive, t(:oh_yeah), t(:role_added, role: params[:role_contract_new][:roles].humanize.titleize, user: result["model"].username)) ) if result.success?
    redirect_back(fallback_location: dashboard_path, flash: flashy(:negative, "", flash_errors(result["contract.default"]) ))
  end
